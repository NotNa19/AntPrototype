...
void setBodiesPlayer(JointWheelPtr joint, const BodyRigidPtr &car_body, const BodyRigidPtr &wheel_body)
{
...
ObjectMeshDynamicPtr createBodyPlayer(char *name, const vec3& size, float mass, const vec4& color, const Mat4& transform)
{
...
ObjectMeshDynamicPtr createWheelPlayer(NodePtr frame, char *name, float radius, float height, const Mat4& transform)
{
...
int CarPlayer::init(int num, vec3 position, float blength, float bwidth, float bheight, float wradius, float wwidth, float wmass, const Mat4& transform)
{
    ...
    car_frame = createBodyPlayer("car_frame", vec3(frame_width, frame_length, frame_height), 64.0f, vec4::GREEN, transform);

    // initialization of wheels 
    wheels[0] = createWheelPlayer(car_frame, "car_wheel_f_l", wheel_radius, wheel_width, Mat4(translate(-(frame_width + wheel_width) / 2 - delta, frame_length / 2 - wheel_radius, -frame_height / 2)* rotateY(90.0f)));
    wheels[1] = createWheelPlayer(car_frame, "car_wheel_f_r", wheel_radius, wheel_width, Mat4(translate((frame_width + wheel_width) / 2 + delta, frame_length / 2 - wheel_radius, -frame_height / 2)* rotateY(90.0f)));

    wheels[2] = createWheelPlayer(car_frame, "car_wheel_r_l", wheel_radius, wheel_width, Mat4(translate(-(frame_width + wheel_width) / 2 - delta, -0.25f * frame_length, -frame_height / 2) * rotateY(90.0f)));
    wheels[3] = createWheelPlayer(car_frame, "car_wheel_r_r", wheel_radius, wheel_width, Mat4(translate((frame_width + wheel_width) / 2 + delta, -0.25f * frame_length, -frame_height / 2) * rotateY(90.0f)));
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        setBodiesPlayer(wheel_joints[i], car_frame->getBodyRigid(), wheels[i]->getBodyRigid());
